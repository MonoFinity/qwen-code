...
if (!qwenSandbox) {
  // Try to read from .qwen/.env first, then .env
  let currentDir = process.cwd();
  while (currentDir !== '/' && currentDir !== path.parse(currentDir).root) {
    // First try project .qwen/.env
    ...
    // Then try normal .env
    const normalEnv = join(currentDir, '.env');
    if (existsSync(normalEnv)) {
      dotenv.config({ path: normalEnv, quiet: true });
      break;
    }
